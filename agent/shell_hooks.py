...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`zoe_cli.plugins.invoke_hook` and its aggregators.  Python
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``zoe_cli.config.load_config``
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`zoe_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`zoe_cli.plugins.invoke_hook`
