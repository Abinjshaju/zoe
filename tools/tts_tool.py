...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``zoe_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from zoe_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    from zoe_constants import get_hermes_dir
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("zoe_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
