...
# IMPORTANT: zoe_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``zoe_bootstrap`` is a
...
# new code references ``zoe_bootstrap`` but the editable install's
...
try:
    import zoe_bootstrap  # noqa: F401
...
# that run *before* zoe_cli.config is importable. Mirrors the explicit
...
def _print_fast_version_info() -> None:
    from zoe_cli import __release_date__, __version__
...
from zoe_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from zoe_cli.subcommands.cron import build_cron_parser
from zoe_cli.subcommands.profile import build_profile_parser
from zoe_cli.subcommands.model import build_model_parser
from zoe_cli.subcommands.setup import build_setup_parser
from zoe_cli.subcommands.postinstall import build_postinstall_parser
from zoe_cli.subcommands.login import build_login_parser
from zoe_cli.subcommands.logout import build_logout_parser
from zoe_cli.subcommands.auth import build_auth_parser
from zoe_cli.subcommands.status import build_status_parser
from zoe_cli.subcommands.webhook import build_webhook_parser
from zoe_cli.subcommands.hooks import build_hooks_parser
from zoe_cli.subcommands.doctor import build_doctor_parser
from zoe_cli.subcommands.security import build_security_parser
from zoe_cli.subcommands.dump import build_dump_parser
from zoe_cli.subcommands.debug import build_debug_parser
from zoe_cli.subcommands.backup import build_backup_parser
from zoe_cli.subcommands.import_cmd import build_import_cmd_parser
from zoe_cli.subcommands.config import build_config_parser
from zoe_cli.subcommands.version import build_version_parser
from zoe_cli.subcommands.uninstall import build_uninstall_parser
from zoe_cli.subcommands.dashboard import build_dashboard_parser
from zoe_cli.subcommands.gui import build_gui_parser
from zoe_cli.subcommands.logs import build_logs_parser
from zoe_cli.subcommands.prompt_size import build_prompt_size_parser
from zoe_cli.subcommands.memory import build_memory_parser
from zoe_cli.subcommands.tools import build_tools_parser
from zoe_cli.subcommands.insights import build_insights_parser
from zoe_cli.subcommands.skills import build_skills_parser
from zoe_cli.subcommands.pairing import build_pairing_parser
from zoe_cli.subcommands.plugins import build_plugins_parser
from zoe_cli.subcommands.mcp import build_mcp_parser
from zoe_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors zoe_cli.profiles._PROFILE_ID_RE so we never call
    ...
    if profile_name is None and not os.environ.get("HERMES_S6_SUPERVISED_CHILD"):
        try:
            from zoe_constants import get_default_hermes_root
    ...
    if profile_name is not None:
        try:
            from zoe_cli.profiles import resolve_profile_env
...
from zoe_cli.config import get_hermes_home
from zoe_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE zoe_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            from zoe_cli import managed_scope
...
try:
    from zoe_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from zoe_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if zoe_constants not importable yet
...
from zoe_cli import __version__, __release_date__
...
# existing test monkeypatches (zoe_cli.main._model_flow_*) keep resolving unchanged.
from zoe_cli.model_setup_flows import (
...
def _has_any_provider_configured() -> bool:
    ...
    from zoe_cli.config import get_env_path, get_hermes_home, load_config
    from zoe_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zoe_cli.config import DEFAULT_CONFIG
    ...
    from zoe_cli.auth import PROVIDER_REGISTRY
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from zoe_state import SessionDB
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from zoe_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zoe_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from zoe_cli.config import apply_terminal_config_to_env
    ...
    if code == 42:
        from zoe_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from zoe_cli.kanban_db import get_current_board
...
def _resolve_use_tui(args) -> bool:
    ...
    if getattr(args, "tui", False) or os.environ.get("HERMES_TUI") == "1":
        return True
    try:
        from zoe_cli.config import load_config
...
def cmd_chat(args):
    ...
    try:
        from zoe_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from zoe_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from zoe_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from zoe_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from zoe_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from zoe_constants import find_node_executable, with_hermes_node_path
...
def cmd_whatsapp_cloud(args):
    ...
    from zoe_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from zoe_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from zoe_cli.config import stamp_install_method
    from zoe_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from zoe_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from zoe_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from zoe_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from zoe_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zoe_cli.config import read_raw_config
...
def _clear_stale_openai_base_url():
    ...
    """
    from zoe_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`zoe_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from zoe_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from zoe_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from zoe_cli.config import load_config
    from zoe_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from zoe_cli.auth import _prompt_model_selection
    from zoe_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from zoe_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from zoe_cli.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from zoe_cli.curses_ui import curses_radiolist
...
# downstream call sites read `zoe_cli.main._PROVIDER_MODELS` directly,
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from zoe_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from zoe_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from zoe_cli.config import save_env_value
    from zoe_cli.secret_prompt import masked_secret_prompt
    ...
    from zoe_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zoe_constants import display_hermes_home as _dhh_fn
...
def cmd_login(args):
    ...
    from zoe_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zoe_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zoe_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zoe_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zoe_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from zoe_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from zoe_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zoe_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from zoe_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zoe_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from zoe_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from zoe_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zoe_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zoe_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from zoe_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zoe_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from zoe_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from zoe_cli.banner import format_banner_version_label
    ...
    try:
        from zoe_cli.banner import check_for_updates
        from zoe_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from zoe_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from zoe_cli.uninstall import run_gui_uninstall
    ...
    from zoe_cli.uninstall import run_uninstall
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m zoe_cli.main web` invocation.
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from zoe_logging import setup_logging as _setup_logging_gui
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `zoe_cli.gateway._scan_gateway_pids` and avoids the
...
def _update_via_zip(args):
    ...
    from zoe_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from zoe_cli.model_catalog import seed_cache_from_checkout
...
def _recover_from_interrupted_install() -> None:
    ...
    try:
        ...
        try:
            from zoe_cli.managed_uv import ensure_uv
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from zoe_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from zoe_cli.managed_uv import resolve_uv
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # zoe_cli.config.get_hermes_home to simulate setup failure.
        from zoe_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from zoe_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from zoe_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from zoe_cli.config import recommended_update_command
        from zoe_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from zoe_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        from zoe_constants import get_hermes_home, display_hermes_home
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from zoe_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _cmd_update_pip(args):
    ...
    from zoe_cli import __version__
    from zoe_cli.config import is_uv_tool_install
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            from zoe_cli.backup import create_quick_snapshot
        ...
        # the old bytecode (e.g. get_hermes_home added to zoe_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        from zoe_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "zoe_cli.main", "desktop", "--build-only"]
        ...
        # modules in this process.  Reload zoe_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hermes_home() added since the last release.
        try:
            import importlib
            import zoe_constants as _hc
        ...
        try:
            from zoe_cli.profiles import backfill_profile_envs
        ...
        try:
            from zoe_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from zoe_cli.tools_config import install_cua_driver
...
def cmd_profile(args):
    ...
    from zoe_constants import display_hermes_home
    ...
    if action == "list":
        ...
        from zoe_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from zoe_constants import get_hermes_home as _hh
        ...
        from zoe_cli import profile_describer as _pd
        ...
        from zoe_cli.profiles import profile_exists
        ...
        from zoe_cli.profiles import rename_profile
        ...
        from zoe_cli.profiles import export_profile
        ...
        from zoe_cli.profiles import import_profile
        ...
        import tempfile
        from zoe_cli.profile_distribution import (
            plan_install,
        ...
        from zoe_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from zoe_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from zoe_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    try:
        from zoe_cli.profiles import get_active_profile_name
    ...
    try:
        from zoe_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    try:
        from zoe_cli.mcp_startup import start_background_mcp_discovery
    ...
    from zoe_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from zoe_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from zoe_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from zoe_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from zoe_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from zoe_cli.logs import tail_log, list_logs
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from zoe_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from zoe_cli.oneshot import run_oneshot
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from zoe_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from zoe_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from zoe_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from zoe_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from zoe_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from zoe_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from zoe_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from zoe_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from zoe_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from zoe_cli.claw import claw_command
...
def main():
    ...
    try:
        from zoe_cli.stdio import configure_windows_stdio
    ...
    # =========================================================================
    from zoe_cli.fallback_cmd import cmd_fallback
    ...
    from zoe_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from zoe_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from zoe_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from zoe_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from zoe_cli.kanban import build_parser as _build_kanban_parser
    ...
    from zoe_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    from zoe_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from zoe_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zoe_cli.curator import register_cli as _register_curator_cli
    ...
    from zoe_cli.config import get_container_exec_info
