...
from zoe_cli.config import get_project_root, get_hermes_home, get_env_path
...
from zoe_cli.colors import Colors, color
from zoe_cli.models import _HERMES_USER_AGENT
from zoe_constants import OPENROUTER_MODELS_URL
...
from zoe_constants import is_termux as _is_termux
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from zoe_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from zoe_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from zoe_cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches zoe_cli.__version__.
    ...
    """
    try:
        from zoe_cli import __version__ as init_version
    except Exception:
        return
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    """
    try:
        from zoe_cli.service_manager import (
            S6ServiceManager,
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        ...
        from zoe_cli.service_manager import detect_service_manager
    except Exception as e:
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from zoe_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from zoe_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from zoe_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from zoe_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from zoe_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from zoe_cli.config import check_config_version, migrate_config
        ...
        try:
            import yaml
            from zoe_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
        ...
        try:
            from zoe_cli.config import validate_config_structure
    ...
    if state_db_path.exists():
        try:
            ...
            from zoe_state import is_malformed_db_error, repair_state_db_schema
    ...
    try:
        from zoe_constants import is_container as _is_container
    ...
    try:
        from zoe_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
