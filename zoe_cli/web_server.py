...
Usage:
    python -m zoe_cli.main web          # Start on http://127.0.0.1:9119
    python -m zoe_cli.main web --port 8080
"""
...
from zoe_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
#
# This list is defined in ``zoe_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from zoe_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from zoe_cli.dashboard_auth.middleware import gated_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from zoe_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from zoe_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from zoe_cli.models import _AGGREGATOR_PROVIDERS
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    """
    try:
        from zoe_constants import is_container
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from zoe_cli.dashboard_auth import list_providers as _list_providers
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from zoe_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from zoe_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from zoe_cli.debug import build_debug_share
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``zoe_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "zoe_cli.main", *subcommand]
...
async def update_hermes():
    ...
    return {
        ...
        "message": "Automatic updates are disabled in this version.",
        "update_command": "",
    }
...
async def check_hermes_update(force: bool = False):
    """Updates are disabled in Zoe v0.1 — always report up-to-date."""
...
async def get_profiles_sessions(
    ...
    from zoe_state import SessionDB
    from zoe_cli import profiles as profiles_mod
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from zoe_cli.auth import get_provider_auth_state
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from zoe_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from zoe_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from zoe_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from zoe_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        pass
    try:
        from zoe_cli.config import get_env_value
    except ImportError:
...
def _gemini_cli_status() -> Dict[str, Any]:
    ...
    try:
        from zoe_cli import auth as hauth
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            ...
            try:
                from zoe_cli.auth import clear_provider_auth
        ...
        try:
            from zoe_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from zoe_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from zoe_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from zoe_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from zoe_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
#
# Wraps the same config data layer the CLI uses (zoe_cli.mcp_config), so
...
async def list_mcp_servers(profile: Optional[str] = None):
    from zoe_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from zoe_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from zoe_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from zoe_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from zoe_cli import mcp_catalog
    except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (zoe_cli.webhook); the webhook
...
async def list_credential_pool():
    ...
    from zoe_cli.auth import read_credential_pool
...
async def list_hooks():
    ...
    form can offer them.
    """
    from zoe_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from zoe_cli.plugins import VALID_HOOKS
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from zoe_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from zoe_constants import set_hermes_home_override, reset_hermes_home_override
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from zoe_constants import set_hermes_home_override, reset_hermes_home_override
    from zoe_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from zoe_constants import set_hermes_home_override, reset_hermes_home_override
    from zoe_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from zoe_cli import profile_describer
...
async def get_skills(profile: Optional[str] = None):
    ...
    from zoe_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from zoe_cli.tools_config import (
...
if sys.platform.startswith("win"):
    try:
        from zoe_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from zoe_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from zoe_cli.dashboard_auth.audit import AuditEvent, audit_log
        from zoe_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``zoe_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from zoe_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from zoe_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`zoe_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from zoe_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as zoe_cli.plugins):
    ...
    from zoe_cli.plugins import get_bundled_plugins_dir
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from zoe_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from zoe_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zoe_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zoe_cli.plugins_cmd import dashboard_remove_user_plugin
...
from zoe_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from zoe_cli.dashboard_auth import list_providers
