...
This is DISTINCT from ``zoe_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through zoe_cli.config.load_config) to honor managed
    scope. Mirrors zoe_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from zoe_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
