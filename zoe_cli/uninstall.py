...
def uninstall_gateway_service():
    ...
    try:
        from zoe_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zoe_cli.gateway import get_launchd_plist_path
...
def _discover_named_profiles():
    ...
    default root."""
    try:
        from zoe_cli.profiles import list_profiles
    except Exception:
        return []
    try:
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m zoe_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "zoe_cli.main", "--profile", name]
...
def run_gui_uninstall(args):
    ...
    """
    from zoe_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
...
def _perform_uninstall(
    *,
    ...
    try:
        from zoe_cli.gui_uninstall import uninstall_gui
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m zoe_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import zoe_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``zoe_constants`` + ``zoe_cli.colors``
    (and lazily ``zoe_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m zoe_cli.uninstall")
    parser.add_argument(
        "--mode",
