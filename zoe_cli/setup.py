...
from zoe_constants import get_optional_skills_dir
...
from zoe_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from zoe_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zoe_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from zoe_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from zoe_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if disabled_tools:
        ...
        from zoe_constants import display_hermes_home as _dhh
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from zoe_cli.main import select_provider_and_model
    try:
...
def _setup_telegram_auto_result():
    ...
    try:
        from zoe_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_qqbot():
    ...
    from zoe_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from zoe_cli.gateway import _all_platforms, _platform_status, _configure_platform
            ...
            if supports_systemd and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd:
                        ...
                        from zoe_cli import gateway_windows
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``zoe_cli.auth`` — lists every supported
    ...
    """
    try:
        from zoe_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zoe_cli.config
    ...
    if section_key == "model":
        ...
        from zoe_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from zoe_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from zoe_cli.config import is_managed, managed_error
    if is_managed():
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from zoe_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
