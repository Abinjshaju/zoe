...
they were not: the CLI picker read :data:`zoe_cli.models.CANONICAL_PROVIDERS`
...
* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`zoe_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
# zoe_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
def provider_catalog() -> list[ProviderDescriptor]:
    ...
    try:
        from zoe_cli.config import OPTIONAL_ENV_VARS
    except Exception:
        OPTIONAL_ENV_VARS = {}
