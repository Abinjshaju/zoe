...
- ``zoe_cli.providers``        -- canonical provider identity + overlays
- ``zoe_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zoe_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from zoe_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from zoe_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from zoe_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in zoe_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from zoe_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from zoe_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zoe_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from zoe_cli.auth import _load_auth_store
    ...
    from zoe_cli.providers import HERMES_OVERLAYS
    from zoe_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from zoe_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from zoe_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`zoe_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from zoe_cli.models import fetch_openrouter_models
