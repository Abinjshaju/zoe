    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See zoe_state._WAL_INCOMPAT_MARKERS for detection logic.
                from zoe_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from zoe_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from zoe_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from zoe_cli.profiles import profile_exists as _pe
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``zoe_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``hermes`` package — there is no
    # ``hermes`` package to import.
    return [sys.executable, "-m", "zoe_cli.main"]
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m zoe_cli.main`` — fallback for setups where
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    if not hermes_home:
        return None
    try:
        from zoe_constants import reset_hermes_home_override, set_hermes_home_override
        from zoe_cli.config import load_config
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* zoe_constants is imported.
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``zoe_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from zoe_constants import get_default_hermes_root
