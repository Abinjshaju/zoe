...
from zoe_cli.default_soul import DEFAULT_SOUL_MD
...
def _running_in_container() -> bool:
    """Thin wrapper around ``zoe_constants.is_container`` (import-safe)."""
    try:
...
# Re-export from zoe_constants — canonical definition lives there.
from zoe_constants import get_hermes_home  # noqa: F811,E402
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zoe_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from zoe_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
