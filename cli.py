...
# IMPORTANT: zoe_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See zoe_bootstrap.py for full rationale.
try:
    import zoe_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when zoe_bootstrap isn't registered in the venv
...
from zoe_cli.fallback_config import get_fallback_chain
from zoe_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from zoe_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from zoe_cli.pt_input_extras import (
...
from zoe_cli.banner import _format_context_length, format_banner_version_label
...
from zoe_cli.browser_connect import (
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from zoe_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from zoe_cli.config import _normalize_root_model_keys
    ...
    from zoe_cli.config import _expand_env_vars
    ...
    # zoe_cli.config._load_config_impl (which has its own managed merge), so
...
try:
    from zoe_logging import setup_logging
...
try:
    from zoe_cli.config import print_config_warnings
...
try:
    from zoe_cli.skin_engine import init_skin_from_config
...
def get_tool_definitions(*args, **kwargs):
    from zoe_cli.mcp_startup import wait_for_mcp_discovery
...
from zoe_cli.banner import build_welcome_banner
from zoe_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from zoe_cli.callbacks import prompt_for_secret
...
def _notify_session_finalize(
    *,
    ...
    try:
        from zoe_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`zoe_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from zoe_cli.config import load_config as _load_full_config
        from zoe_constants import get_hermes_home as _get_hermes_home
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`zoe_cli.config.load_config`. Honours ``auto_prune`` /
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from zoe_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from zoe_cli.skin_engine import get_active_skin
...
from zoe_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from zoe_cli import __release_date__ as _release_date
        from zoe_cli import __version__ as _version
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from zoe_cli.plugins import get_plugin_commands
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from zoe_cli.runtime_provider import _auto_detect_local_model
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from zoe_cli.active_sessions import try_acquire_active_session
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from zoe_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from zoe_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from zoe_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from zoe_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from zoe_cli.codex_models import get_codex_model_ids
    ...
    def _show_security_advisories(self):
        ...
        """
        try:
            from zoe_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        from zoe_cli.model_switch import is_nous_hermes_non_agentic
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        """
        from zoe_cli.clipboard import save_clipboard_image
    ...
    def _fast_command_available(self) -> bool:
        try:
            from zoe_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from zoe_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from zoe_cli.skin_engine import get_active_help_header
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from zoe_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from zoe_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        if getattr(self, "conversation_history", None):
            return False
        try:
            from zoe_constants import get_hermes_home as _ghh
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from zoe_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if not getattr(result, "success", False):
            return True
        try:
            from zoe_cli.model_cost_guard import expensive_model_warning
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from zoe_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from zoe_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from zoe_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from zoe_cli.model_switch import (
            switch_model,
            parse_model_flags,
            resolve_persist_behavior,
        )
        from zoe_cli.providers import get_label
        ...
        # clobber disk config.
        from zoe_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from zoe_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from zoe_cli.commands import resolve_command
    ...
    def process_command(self, command: str) -> bool:
        ...
        from zoe_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from zoe_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from zoe_cli.skin_engine import get_active_skin
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from zoe_state import format_session_db_unavailable
            ...
            from zoe_cli.main import _print_version_info
            ...
            from zoe_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                from zoe_cli.plugins_cmd import (
                ...
                if not user_entries:
                    ...
                    loaded: dict = {}
                    try:
                        from zoe_cli.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from zoe_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from zoe_cli.goals import GoalManager
            from zoe_cli.config import load_config
        except Exception as exc:
    ...
    def _toggle_verbose(self):
        ...
        from zoe_cli.colors import Colors as _Colors
        labels = {
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from zoe_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by zoe_logging not
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from zoe_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from zoe_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if choice != "yes":
            ...
            return
        try:
            from zoe_cli.auth import step_up_nous_billing_scope
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from zoe_cli.config import get_config_path as _get_config_path
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from zoe_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from zoe_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from zoe_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            from zoe_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from zoe_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in zoe_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from zoe_cli.model_switch import resolve_persist_behavior
...
def _run_kanban_goal_loop_q(cli: "HermesCLI", first_response: str) -> None:
    ...
    from zoe_cli import kanban_db as _kb
    from zoe_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    if toolsets:
        ...
        if _coding is not None:
            ...
            from zoe_cli.tools_config import _get_platform_tools
