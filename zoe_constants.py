...
_UNSET = object()
_ZOE_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ZOE_HOME_OVERRIDE", default=_UNSET
)
...
def _get_platform_default_hermes_home() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return base / "hermes"
    return Path.home() / ".zoe"


def get_hermes_home() -> Path:
    """Return the Hermes home directory (default: platform-native path).

    Reads ZOE_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``ZOE_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``ZOE_HOME`` explicitly (see the systemd
    ...
    val = os.environ.get("ZOE_HOME", "").strip()
    if val:
        return Path(val)
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            msg = (
                f"[ZOE_HOME fallback] ZOE_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass ZOE_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_hermes_root() -> Path:
    ...
    (``~/.zoe`` on POSIX, ``%LOCALAPPDATA%\\hermes`` on native Windows).

    In Docker or custom deployments where ``ZOE_HOME`` points outside
    ``~/.zoe`` (e.g. ``/opt/data``), returns ``ZOE_HOME`` directly
    — that IS the root.

    In profile mode where ``ZOE_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.zoe/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_hermes_home()
    env_home = os.environ.get("ZOE_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # ZOE_HOME is under ~/.zoe (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ZOE_HOME itself is the root
    return env_path
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to ZOE_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ZOE_HOME (e.g. ``"image_cache"``).
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    ``$ZOE_HOME/node/bin``. Include both shapes on every platform so mixed
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current ZOE_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.zoe``
        profile:  ``~/.zoe/profiles/coder``
        custom:   ``/opt/hermes-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.zoe``.  For code that needs a real ``Path``, use
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ZOE_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ZOE_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get("ZOE_HOME") or os.getenv("ZOE_HOME")
    if not hermes_home:
        return None
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    ``ZOE_HOME`` scopes Hermes state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ZOE_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ZOE_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ZOE_HOME}/home`` when it exists, preserving the
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ZOE_HOME.

    Replaces the ``get_hermes_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, zoe_logging.py, zoe_time.py, etc.).
    """
    return get_hermes_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ZOE_HOME."""
    return get_hermes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ZOE_HOME."""
