...
``zoe_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
from zoe_cli import kanban_db
from zoe_cli import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``zoe_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from zoe_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``zoe_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from zoe_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from zoe_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from zoe_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See zoe_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from zoe_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from zoe_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from zoe_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from zoe_cli import kanban_decompose  # noqa: WPS433 (intentional)
